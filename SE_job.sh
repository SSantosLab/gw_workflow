...
if [ $# -lt 1 ]; then
    echo "usage: SE_job.sh -E EXPNUM -r RNUM -p PNUM -n NITE -b BAND [-j] [-s] [-m SCHEMA (gw or wsdiff)] [-Y] [-C] [-d destcache]" 
    exit 1
fi
...
ARGS="$@"
while getopts "E:n:b:r:p:d:CjhsYOm:" opt $ARGS
do case $opt in
    E)
    ...
    O)
	    ...
	    ;;
    h)
	    echo "usage: SE_job.sh -E EXPNUM -r RNUM -p PNUM -n NITE -b BAND [-j] [-s] [-S scratch|persistent] [ -m gw|wsdiff] [-Y] [-C] [-O]" 
	    exit 1
            ;;
    :)
...
ifdh cp --force=xrootd /pnfs/des/scratch/${SCHEMA}/db-tools/desservices.ini ${HOME}/.desservices.ini
