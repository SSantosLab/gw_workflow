#!/usr/bin/env python
#
# SE processing program using multiprocessing pool
#
# 
# By: N. kuropatkin 
#
...
def l1p(inargs):
    ...
    confPars = inargs[1]
    ccdstring = inargs[2]
    pixcorrect('detrend',confPars,ccdstring,**args)
    nullweight('detrend', 'nullweight',confPars, ccdstring,**args)
    sextractor('nullweight', 'sextractor', confPars,ccdstring,**args )
    return 0

def link_from_Dcache(fileN):
        print "link file %s \n" % fileN
        dest = fileN.split('/')[-1]
...
def pixcorrect(outname, confPars,CCD, **args):
        args['ccd']=CCD    
        year = args['year']
        epoch = args['epoch']
        linearity = confPars['linearity']
        template_file = confPars['template_file']
        correc_dir = confPars['correc_dir']
        bias = confPars['bias']
        bpm =  confPars['bpm']
        bf = confPars['bf']
        flat = confPars['flat']
#        print "CCD=%s linearity=%s bpm=%s flat=%s correc_dir =%s \n" % (CCD,linearity,bpm,flat,correc_dir)
#
        link_from_Dcache(correc_dir+'/biascor/'+str(year)+str(epoch)+'/'+ bias.format(**args))

#
        link_from_Dcache(correc_dir+'/norm_dflatcor/'+str(year)+str(epoch)+'/'+flat.format(**args) )

#
        link_from_Dcache(correc_dir+'/bpm/'+str(year)+str(epoch)+'/'+bpm.format(**args))
...
def nullweight( inname, outname,confPars,CCD,**args):
    args['ccd']=CCD
    template_file = confPars['template_file']
...
def sextractor(inname, outname, confPars, CCD, **args):
    args['ccd']=CCD
    template_file = confPars['template_file']
    sexnnwFile = confPars['sexnnwFile']
    sexconvFile = confPars['sexconvFile']
    sexparamFile = confPars['sexparamFile']
    confPars['sexparamPSFEXFile']
    configFile = confPars['configFile']
...
def nullweightbkg(inname, outname, CCD, confPars, **args):
...
def l2p(args):
    ...
    confPars = args[1]
    ccdstring = args[2]
    bleedmask(ccdstring,confPars,'wcs','bleedmasked',**inpars)
    skycompress(ccdstring,confPars,'bleedmasked','bleedmask-mini',**inpars)
    return 0
...
def bleedmask(CCD,confPars,inname,outname,**args):
    args['ccd']=CCD
    template_file = confPars['template_file']
...
def skycompress(CCD,confPars,inname,outname,**args):
        args['ccd']=CCD
        template_file = confPars['template_file']
...
def l3p(args):
    ...
    confPars = args[1]
    ccdstring = args[2]
    skysubtract(ccdstring,confPars,'bleedmasked','skysub','skyfit-binned-fp',**inargs )
    pixcorr_starflat('skysub', 'starflat', ccdstring,confPars,**inargs)
    nullweightbkg('starflat','nullwtbkg',ccdstring,confPars,**inargs)
    sextractorsky('nullwtbkg','bkg',ccdstring,confPars,**inargs)
    immask(ccdstring,confPars,'starflat','bkg','immask',**inargs) 
    rowinterp_nullweight('immask', 'nullweightimmask', ccdstring,confPars,**inargs)
    sextractorPSFEX('nullweightimmask', 'sextractorPSFEX', ccdstring,confPars,**inargs )
#    se.immask(ccdstring,'starflat','immask',**args)
    psfex( 'sextractorPSFEX', ccdstring,confPars, **inargs)
    sextractorPSF('nullweightimmask', 'nullweightimmask', 'fullcat', 'sextractorPSFEX.psf', ccdstring,confPars,**inargs)    
    return 0
...
def skysubtract(CCD, confPars, inname, outname, skyfitinfoFile,**args ):
        args['ccd']=CCD
#
        correc_dir = confPars['correc_dir']
        pc_filename = confPars['pc_filename']
        template_file = confPars['template_file']
        exp_template_file = confPars['exp_template_file']
        flat = confPars['flat']
        weight = confPars['weight']
        year = args['year']
        epoch = args['epoch']
        link_from_Dcache(correc_dir+'/skypca/'+str(year)+str(epoch)+'/skytemp/'+ pc_filename.format(**args))
...
def pixcorr_starflat(inname, outname, CCD, confPars, **args):
    args['ccd']=CCD
    correc_dir = confPars['correc_dir']
    template_file = confPars['template_file']
    starflat = confPars['starflat']
    year = args['year']
    epoch = args['epoch']  
    link_from_Dcache(correc_dir+'/starflat/'+str(year)+str(epoch)+'/'+starflat.format(**args))
...
def sextractorsky(inname, outname, CCD, confPars,  **args):
    args['ccd']=CCD
    template_file = confPars['template_file']
    sexnnwFile = confPars['sexnnwFile']
    sexconvFile = confPars['sexconvFile']
    ...
    confPars['sexparamPSFEXFile']
    configFile = confPars['configFile']
    sexbkgparamFile = confPars['sexbkgparamFile']
    cmd = 'sex ' + template_file.format(**args)+'_'+inname+'.fits[0]'+\
...
#----------------------------------------#
def immask(CCD, confParams, inname,bkgname, outname, **args):
    args['ccd']=CCD
    template_file = confPars['template_file']
...
def rowinterp_nullweight(inname, outname,CCD, confPars, **args):
    args['ccd']=CCD
    template_file = confPars['template_file']
...
def sextractorPSFEX( inname, outname, CCD, confPars, **args):
    args['ccd']=CCD
    template_file = confPars['template_file']
    sexnnwFile = confPars['sexnnwFile']
    sexconvFile = confPars['sexconvFile']
    confPars['sexparamPSFEXFile']
    sexparamPSFEXFile = confPars['sexparamPSFEXFile']
    confPSF = confPars['confPSF']
    cmd = 'sex ' + template_file.format(**args)+'_'+inname+'.fits[0]'+\
...
def psfex( name,CCD, confPars, **args):
    args['ccd']=CCD
    template_file = confPars['template_file']
    config_filePSF = confPars['config_filePSF']
...
def sextractorPSF( name, name1, outname, filepsf, CCD, confPars, **args):
    args['ccd']=CCD
    template_file = confPars['template_file']
    sexnnwFile = confPars['sexnnwFile']
    sexconvFile2 = confPars['sexconvFile2']
    confPars['sexparamPSFEXFile']
    sexparamFile_2 = confPars['sexparamFile_2']
    configFile2 = confPars['configFile2']
...
#
    def getConfVals(self):
        confPars = {}
        confPars['template_file']=self.template_file
        confPars['exp_template_file']=self.exp_template_file
        confPars['bias']=self.bias
        confPars['bpm']= self.bpm 
        confPars['linearity']=self.linearity
        confPars['bf']=self.bf
        confPars['flat']=self.flat
        " sextractor  "
        confPars['sexnnwFile']=self.sexnnwFile
        confPars['sexconvFile'] = self.sexconvFile
        confPars['sexparamFile'] = self.sexparamFile
        confPars['sexparamPSFEXFile'] = self.sexparamPSFEXFile
        confPars['configFile'] = self.configFile
        confPars['confPSF'] = self.confPSF
        confPars['correc_dir'] = self.correc_dir
        confPars['chiplist'] = self.chiplist
        " psfex "
        confPars['config_filePSF'] = self.config_filePSF
        " skysubtract "
        confPars['pc_filename'] = self.pc_filename
        confPars['weight'] =self.weight
        " sextractor for PSFEX "
        confPars['sexparamPSFEXFile'] = self.sexparamPSFEXFile
        confPars['config_filePSF'] = self.config_filePSF
        " sextractor with PSF "
        confPars['sexparamFile_2'] = self.sexparamFile_2 
        confPars['configFile2'] = self.configFile2
        confPars['sexconvFile2'] = self.sexconvFile2
        confPars['starflat'] = self.starflat
        confPars['sexbkgparamFile'] = self.sexbkgparamFile
        return confPars
