...
do case $opt in
    E)
	    ...
	    EXPNUM_REGEX='^([0-9][0-9]*|([0-9][0-9]*,?)+[0-9][0-9]*)$'
            [[ $OPTARG =~ $EXPNUM_REGEX ]] || { echo "Error: exposure numbers must be integers! You put $OPTARG" ; exit 1; }
	    EXPNUM_LIST=$OPTARG #TODO export?
            shift 2
            ;;
    n)
...
if [ "x$EXPNUM_LIST" = "x" ]; then echo "Exposure number not set; exiting." ; exit 1 ; fi
...
##############################################################################################################################
##############################################################################################################################
## CODE FOR EXPOSURE LIST STARTS HERE
##############################################################################################################################
##############################################################################################################################
# for each exposure from the list
for EXPNUM in $(echo $EXPNUM_LIST | sed "s/,/ /g"); do
    # get filenames
...
##############################################################################################################################
##############################################################################################################################
## DONE WITH EXPOSURE LIST -- NOW JUST FIRST EXPNUM GOES ON ##################################################################
##############################################################################################################################
##############################################################################################################################

# first element from EXPNUM_LIST is EXPNUM now on
EXPNUM=$(echo $EXPNUM_LIST | cut -d"," -f1)
...
#ok, now check the psf and csv files, but only if we need them
	...
	if [ ${tempexp} == ${EXPNUM} ]; then continue ; fi
