...
#----------------------------------------------------------#
    ...
    def crosstalk(self,EXPFILE,NITE,**args):
        ...
        self.copy_from_Dcache(self.data_conf+self.xtalk_file) #copy xtalk file
        self.copy_from_Dcache(self.data_dir + NITE + '/' + EXPFILE)  #copy data
        self.copy_from_Dcache(self.data_conf+self.replace_file)
...
#    copy_from_Dcache(correc_dir+'superflat_'+str(year)+'_'+str(epoch)+'/biascor/'+bias.format(**args))
        self.copy_from_Dcache(self.correc_dir+'/biascor/'+str(self.year)+str(self.epoch)+'/'+self.bias.format(**args))

#    copy_from_Dcache(correc_dir+'superflat_'+str(year)+'_'+str(epoch)+'/norm-dflatcor/'+flat.format(**args) )
        self.copy_from_Dcache(self.correc_dir+'/norm_dflatcor/'+str(self.year)+str(self.epoch)+'/'+self.flat.format(**args) )

#    copy_from_Dcache(correc_dir+'bpm_'+str(yearb)+'_'+str(epochb)+'/'+bpm.format(**args))
        self.copy_from_Dcache(self.correc_dir+'bpm/'+str(self.year)+str(self.epoch)+'/'+self.bpm.format(**args))
...
#----------------------------------------#      
    ...
    def pixcorr_starflat(self,inname, outname, CCD,**args):
        args['ccd']=CCD
        self.copy_from_Dcache(self.correc_dir+'/starflat/'+str(self.year)+str(self.epoch)+'/'+self.starflat.format(**args))
...
#    weight = ConfigSectionMap("skysubtract")['weight']
    ...
    def skysubtract(self,CCD, inname, outname, skyfitinfoFile,**args ):
        args['ccd']=CCD
#
        self.copy_from_Dcache(self.correc_dir+'/skypca/'+str(self.year)+str(self.epoch)+'/skytemp/'+self.pc_filename.format(**args))
...
#running pixelcorrect ans bleedmask
    se.copy_from_Dcache(se.data_conf+'default.psf')
    for ccd in CCD:
        ccdstring= "%02d"%int(ccd)
        se.pixcorrect('detrend',ccdstring,**args)
        se.nullweight('detrend', 'nullweight', ccdstring,**args)
        se.sextractor('nullweight', 'sextractor', ccdstring, **args)
...
#
    for ccd in CCD:
        ccdstring= "%02d"%int(ccd)
        se.bleedmask(ccdstring,'wcs','bleedmasked',**args)
        se.skycompress(ccdstring,'bleedmasked','bleedmask-mini',**args)
...
# 
    for ccd in CCD:
        ccdstring= "%02d"%int(ccd)
        se.skysubtract(ccd,'bleedmasked','skysub','skyfit-binned-fp',**args )
        se.pixcorr_starflat('skysub', 'starflat', ccdstring,**args)
        se.nullweightbkg('starflat','nullwtbkg',ccdstring,**args)
        se.sextractorsky('nullwtbkg','bkg',ccdstring,**args)
#        se.immask(ccdstring,'starflat','immask',**args)
        se.immask(ccdstring,'starflat','bkg','immask',**args) 
        se.rowinterp_nullweight('immask', 'nullweightimmask', ccdstring,**args)
        se.sextractorPSFEX('nullweightimmask', 'sextractorPSFEX', ccdstring, **args)
